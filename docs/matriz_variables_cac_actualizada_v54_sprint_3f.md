...
**Estado del documento:** En construcción / actualización progresiva
**Variables documentadas:** V1-V134 en matriz funcional base
**Bloque técnico actual:** Sprint 3F · Módulo 12A · V54
**Próxima variable técnica a implementar:** V54
...
### V54 — Medicamento antineoplásico o terapia hormonal adicional 1 — primer o único esquema

**Sprint técnico:** Sprint 3F  
**Módulo técnico del validador:** Módulo 12A  
**Archivo de reglas sugerido:** `src/validaciones/reglas/modulo12.js`  
**Versión sugerida:** `sprint-3f-v54-medicamento-adicional-01`

**Módulo CAC:** Terapia sistémica — primer o único esquema  
**Tipo de dato:** Código ATC / Comodín  
**Formato:** Código ATC del medicamento  
**Catálogo de referencia:** Listado ATC disponible en SISCAC, cargado localmente en el validador.

**Descripción oficial:**

Medicamento antineoplásico o terapia hormonal para el cáncer, adicional a los reportados en las variables V53.1 a V53.9 — medicamento adicional 1 administrado al usuario — primer o único esquema del periodo de reporte.

Registrar el código ATC del medicamento. No se tiene en cuenta la cobertura del plan de beneficios en salud.

**Comodines permitidos:**

| Código | Cuándo aplica |
|---|---|
| `97` | No aplica: no recibió medicamentos diferentes a los enunciados en V53.1 a V53.9 y en V45 seleccionó la opción `1`. |
| `98` | No aplica: no tuvo este esquema y en V45 seleccionó la opción `98`. |

**Dependencias:**

| Variable | Relación |
|---|---|
| V45 | Controla si el bloque de terapia sistémica aplica. |
| V53.1-V53.9 | V54 no debe repetir medicamentos ya registrados en estas variables. |
| V55-V56 | Cruce futuro para evitar repetición entre medicamentos adicionales. |
| Catálogo ATC | Permite validar existencia del código ATC cargado. |
| Variables del segundo/último esquema | Cruce futuro para evitar capturar el mismo esquema dos veces. |

**Reglas de validación activas para V54:**

1. V54 no debe estar vacía.
2. V54 debe ser un código ATC, `97` o `98`.
3. Si V45 = `1`, V54 no puede ser `98`.
4. Si V45 = `98`, V54 debe ser `98`.
5. Si V54 tiene formato ATC, debe existir en el catálogo ATC cargado.
6. Si V54 es un código ATC, no debe estar repetido en V53.1 a V53.9.

**Códigos de hallazgo sugeridos:**

| Código | Regla | Severidad |
|---|---|---|
| `V54-ERROR-001` | V54 está vacía. | Error |
| `V54-ERROR-002` | V54 tiene un valor diferente de código ATC, `97` o `98`. | Error |
| `V54-ERROR-003` | V45 = `1`, pero V54 = `98`. | Error |
| `V54-ERROR-004` | V45 = `98`, pero V54 es diferente de `98`. | Error |
| `V54-ERROR-005` | V54 tiene formato ATC, pero no existe en el catálogo ATC cargado. | Error |
| `V54-ERROR-006` | V54 repite un medicamento ya registrado en V53.1 a V53.9. | Error |

**Reglas que NO se automatizan todavía:**

| Regla del instructivo | Tratamiento actual |
|---|---|
| Reportar únicamente medicamentos antineoplásicos. | No automatizar hasta tener clasificación confiable dentro del catálogo ATC. |
| Revisar que no se repita en V55 y V56. | Se implementará cuando V55 y V56 estén activas. |
| No capturar el mismo esquema de tratamiento en primer y segundo esquema. | Pendiente hasta implementar variables del segundo/último esquema. |
| Soporte documental de administración. | Auditoría manual; no automatizable sin soportes clínicos estructurados. |

**Soporte documental:** Sí — registros de administración del medicamento.

**Severidad:**

- **Error** si: V54 está vacía, tiene formato inválido, contradice V45, no existe en catálogo ATC cargado o repite un ATC ya reportado en V53.1-V53.9.
- **Sin advertencia automática por `97`:** V54 = `97` es válido cuando V45 = `1` y no hubo medicamentos adicionales diferentes a V53.1-V53.9.

---

### V55 — Medicamento antineoplásico o terapia hormonal adicional 2 — primer o único esquema

**Sprint técnico:** Sprint 3F  
**Módulo técnico del validador:** Módulo 12B  
**Estado:** Pendiente de implementación después de cerrar V54.
...
Medicamento antineoplásico o terapia hormonal adicional al bloque V53.1-V53.9 y posterior a V54, administrado al usuario en el primer o único esquema del periodo de reporte.

**Comodines permitidos:**

| Código | Cuándo aplica |
|---|---|
| `97` | No aplica: no recibió medicamentos diferentes a los ya reportados y V45 = `1`. |
| `98` | No aplica: no tuvo este esquema y V45 = `98`. |
...
- V54.
- Catálogo ATC.
- V56 para cruce futuro de duplicidad.

**Reglas previstas:**

1. V55 no debe estar vacía.
2. V55 debe ser código ATC, `97` o `98`.
3. Si V45 = `1`, V55 no puede ser `98`.
4. Si V45 = `98`, V55 debe ser `98`.
5. Si V55 es ATC, debe existir en catálogo ATC cargado.
6. Si V55 es ATC, no debe repetirse en V53.1-V53.9 ni en V54.

**Soporte documental:** Sí — registros de administración.

**Severidad prevista:** Error para formato, aplicabilidad, catálogo y duplicidad.

---

### V56 — Medicamento antineoplásico o terapia hormonal adicional 3 — primer o único esquema

**Sprint técnico:** Sprint 3F  
**Módulo técnico del validador:** Módulo 12C  
**Estado:** Pendiente de implementación después de cerrar V55.
...
Medicamento antineoplásico o terapia hormonal adicional al bloque V53.1-V53.9, V54 y V55, administrado al usuario en el primer o único esquema del periodo de reporte.
...
1. V56 no debe estar vacía.
2. V56 debe ser código ATC, `97` o `98`.
3. Si V45 = `1`, V56 no puede ser `98`.
4. Si V45 = `98`, V56 debe ser `98`.
5. Si V56 es ATC, debe existir en catálogo ATC cargado.
6. Si V56 es ATC, no debe repetirse en V53.1-V53.9, V54 ni V55.
...
| Bloque | Estado técnico actual | Notas |
|---|---|---|
| V1-V16 | Cerrado funcionalmente | Identificación del usuario. Mantener activo salvo bug real. |
| V17-V44 | Cerrado funcionalmente por módulos previos | Diagnóstico, estadificación, HER2, Dukes, Ann Arbor/Lugano, Gleason, riesgo, intervención y antecedente de otro cáncer. |
| V45-V47 | Cerrado funcionalmente | Módulo 9 · Terapia sistémica general, fases y ciclos. |
| V48-V52 | Cerrado funcionalmente | Módulo 10 · Primer o único esquema: ubicación, fecha e IPS. |
| V53-V53.9 | Cerrado funcionalmente | Módulo 11 · Cantidad propuesta y medicamentos ATC 1 a 9. |
| V54 | En preparación | Sprint 3F · Módulo 12A · Medicamento adicional 1. |
| V55 | Pendiente | Sprint 3F · Módulo 12B · Medicamento adicional 2. |
| V56 | Pendiente | Sprint 3F · Módulo 12C · Medicamento adicional 3. |
| V57 en adelante | Pendiente | Continuar solo después de cerrar V54-V56. |
